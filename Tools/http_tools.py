...
from Tools.user_agents import FakeChromeUA
from config import PROXY_REDIS
...
def get_response(url, headers=HEADERS, retry=5, **kwargs):
    while True:
        retry -= 1
        if retry < 0:
            try:
                ...
                logging.info('没有可用的代理{%s}' % e)
                retry += 2
        try:
            response = requests.session().get(url, headers=headers, timeout=(60, 120), **kwargs)
            time.sleep(random.randint(1, 15) / 10)
            if response.status_code == 200:
                return response
            elif response.status_code >= 400:
                logging.info(u'continue and 跟换 headers; code error:%s{%s}' % (response.status_code, url))
                headers["User-Agent"] = ua.get_ua()
                time.sleep(random.randint(10, 30))
                continue
            else:
                logging.info(u'continue and change headers;code error:%s{%s}' % (response.status_code, url))
                continue
        except Exception as e:
            headers["User-Agent"] = ua.get_ua()
            logging.info('conn--error-->请求失败url{%s} %s' % (url, e))
            time.sleep(10)
...
def get_proxy():
    return requests.get(PROXY_REDIS + "get/").text


def delete_proxy(proxy):
    requests.get(PROXY_REDIS + "delete/?proxy={}".format(proxy))


if __name__ == '__main__':
    print(get_response('https://www.baidu.com').status_code)
